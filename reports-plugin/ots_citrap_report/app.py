...
from flask import Blueprint, request, Response, send_file, send_from_directory, current_app as app
...
def _count_payload_attachments(payload: bytes) -> int:
    """
    Count files bundled directly inside a report's own payload zip (photos,
    videos, etc.) - distinct from CitrapAttachment rows added via the
    separate addAttachment API call.

    Real EUD submissions often bundle photos straight into the report's own
    data-package zip rather than issuing separate addAttachment calls per
    file - the report detail page's image/video gallery already renders
    these correctly (via _list_extra_zip_files), but the list page's
    "Attachments" column and the /data JSON endpoint previously only
    counted CitrapAttachment rows, showing "0" for a report a user could
    see had two photos the moment they opened it. This makes both counts
    consistent with what the detail page actually displays.
    """
    if not payload:
        return 0
    report_xml_filename = _find_report_xml_filename(payload)
    return len(_list_extra_zip_files(payload, report_xml_filename))
...
def _render_report_list_html(
    ...
    for r in reports:
        ...
        attachment_count = r.attachments.count() + _count_payload_attachments(r.payload)
...
class OtsCitrapReport(Plugin):
    def __init__(self):
        ...
        # A THIRD blueprint for OTS's own official plugin-framework contract:
        # every plugin gets a Settings tab (GET/POST {prefix}/config) and,
        # optionally, a UI tab showing {prefix}/ui in an iframe on the
        # Plugins page, where {prefix} = /api/plugins/<distro> exactly
        # (confirmed against OpenTAKServer-UI's Plugin.tsx: it fetches
        # `/api/plugins/${pluginName}/config` and iframes
        # `/api/plugins/${pluginName}/ui` unconditionally for every plugin,
        # using the distro string from GET /api/plugins). Deliberately
        # separate from self.admin_blueprint above (the existing
        # /api/citrap-reports report browser) rather than replacing it -
        # /ui below just redirects into that existing, already-working page
        # rather than duplicating its logic.
        self.official_blueprint = Blueprint(
            "ots_citrap_report_official", __name__, url_prefix=f"/api/plugins/{DISTRO_NAME}"
        )
        self._register_official_routes()
        self._official_blueprint_registered = False
    ...
    def _register_ui_routes(self, enabled):
        ...
        def ui_data():
            ...
            def _report_summary(r):
                d = r.to_dict(include_payload=False)
                d["attachmentCount"] += _count_payload_attachments(r.payload)
                return d
    ...
    def _register_official_routes(self):
        """
        OTS core's own web UI (OpenTAKServer-UI's Plugin.tsx) fetches
        `GET {prefix}/config` for EVERY plugin's Settings tab, and shows
        `{prefix}/ui` in an iframe for the UI tab - where {prefix} MUST be
        `/api/plugins/<distro>` exactly (confirmed against Plugin.tsx and
        the reference OTS-SkyFi-Plugin). Neither existed on this repo's own
        prefixes (OTS_CITRAP_REPORT_URL_PREFIX is /Marti/*, deliberately
        avoided by nginx on port 443; OTS_CITRAP_REPORT_ADMIN_UI_PREFIX is
        /api/citrap-reports, a fine URL to bookmark directly but not the
        one OTS's Plugins page iframe/Settings-tab fetch actually hits) -
        so neither the Settings tab nor the UI tab worked from inside OTS's
        own Plugins page, even though the report browser worked fine when
        visited directly. This wires both up without touching either
        existing blueprint.

        Auth matches this repo's own established convention for the report
        browser (@auth_required() - any logged-in OTS user, not
        admin-restricted), not the stricter roles_accepted("administrator")
        used elsewhere (e.g. ots-federation) - intentional, to stay
        consistent with how this plugin already gates its other UI routes
        rather than silently changing who can view/manage reports.
        """
        from flask_security import auth_required

        bp = self.official_blueprint

        @bp.route("/config", methods=["GET"], endpoint="citrap_official_config_get")
        @auth_required()
        def official_config_get():
            import ots_citrap_report.default_config as default_config
            return Response(
                json.dumps({
                    key: self._app.config.get(key, getattr(default_config, key))
        ...
        @bp.route("/config", methods=["POST"], endpoint="citrap_official_config_post")
        @auth_required()
        def official_config_post():
            import ots_citrap_report.default_config as default_config
            payload = request.get_json(silent=True) or {}
            ok, error = default_config.validate(payload)
            if not ok:
                return Response(json.dumps({"success": False, "error": error}), status=400, mimetype="application/json")
            self._app.config.update(payload)
            return Response(json.dumps({"success": True, "error": ""}), status=200, mimetype="application/json")

        @bp.route("/ui", methods=["GET"], endpoint="citrap_official_ui")
        @auth_required()
        def official_ui():
            # Serves the static, self-contained ui/index.html shipped in
            # this package (no CDN deps, already fetches report data from
            # the existing /api/citrap-reports/data endpoint and links out
            # to the full admin page) - this file already existed in the
            # repo with exactly this intent, it just had no route serving
            # it at the prefix OTS's iframe actually requests.
            import pathlib
    ...
    def activate(self, app, enabled: bool) -> None:
        ...
        if not self._official_blueprint_registered:
            try:
                app.register_blueprint(self.official_blueprint)
                self._official_blueprint_registered = True
            except AssertionError:
                logger.error(
                    "ots_citrap_report: could not register the official "
                    f"/api/plugins/{DISTRO_NAME} blueprint (Settings/UI tabs) - same "
                    "late-enable timing issue as the admin UI blueprint above; a full "
                    "OTS restart is needed to pick it up."
                )
    ...
    def load_metadata(self) -> dict:
        project_urls = []
        # OTS core's own web UI (Plugin.tsx About tab) renders this under the
        # key "description" (<Markdown>{about?.description}</Markdown>), but
        # deliberately sourced here from the package's short one-line
        # Summary header, NOT the long Description/README payload (which
        # Poetry populates from README.md's full contents via `readme =`).
        # This plugin's README is protocol-documentation-heavy (Marti API
        # paths, CI-TRAP terminology throughout) - showing that verbatim in
        # the About tab would defeat the point of a "Reports"-only display
        # name. Keep this short and Reports-only; don't switch to the full
        # long description without re-checking README.md's wording first.
        summary = "Reports: view, upload, and manage EUD-submitted reports on OpenTAKServer"
        try:
            ...
            summary = dist.metadata.get("Summary") or summary
            ...
            for entry in dist.metadata.get_all("Project-URL") or []:
                if entry not in project_urls:
                    project_urls.append(entry)

            home_page = dist.metadata.get("Home-page")
            if home_page and not any(home_page in u for u in project_urls):
                project_urls.append(f"Homepage, {home_page}")
