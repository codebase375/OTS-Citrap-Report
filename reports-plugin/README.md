...
`ots_citrap_report/ui/index.html` is a single self-contained static HTML
file with inline CSS and vanilla JS - no build step, no external CDN
dependencies. It fetches from `/api/citrap-reports/data` (the existing
JSON endpoint backing the full admin page below, same-origin so the
existing OTS session cookie carries over automatically) and renders a
compact, read-only report table, with a link out to the full admin page.

**Correction from an earlier version of this doc**: this used to claim
"core serves a plugin's `ui/` folder at `/api/plugins/<distro>/ui`
automatically." That's not true - confirmed by reading OTS core's actual
source (`opentakserver/plugins/PluginManager.py`,
`opentakserver/blueprints/ots_api/plugin_api.py`) while chasing the exact
same gap on a sibling plugin (ots-federation): OTS core has no route that
serves a plugin's `ui/` folder. What's actually true is only what
`docs.opentakserver.io/plugins.html` and OpenTAKServer-UI's own `Plugin.tsx`
say: **every plugin's Settings tab fetches `GET/POST /api/plugins/<distro>/config`,
and the UI tab iframes `/api/plugins/<distro>/ui`** - but the *plugin
itself* has to implement both of those routes; nothing serves them for
free. This file existed with exactly the right content and the right
in-package location the whole time, but no route ever served it there -
`app.py`'s `official_blueprint` (`_register_official_routes`) now adds
both, using `send_from_directory` for `/ui` (same pattern as the reference
[OTS-SkyFi-Plugin](https://github.com/brian7704/OTS-SkyFi-Plugin)) and a
plain `default_config`-backed JSON reader/validator for `/config`.

Where the file lives still matters, and is unchanged: `ui/` lives
**inside the Python package folder** (`ots_citrap_report/ui/`), not at the
project root next to `pyproject.toml`, matching `OTS-UI-Plugin-Template`'s
`vite.config.mjs` convention (`outDir: '../<python_package_name>/ui'`) even
though this particular file was hand-written rather than built by that
template. `pyproject.toml`'s `include` is set to
...
`.../OTS-Citrap-Report/ots_citrap_report/ui` (adjusting the relative path
to wherever this plugin's checkout actually lives), build your own
components against `/api/citrap-reports/data`, and `yarn build` will
replace this static file with the real bundled app - no Python-side
changes needed either way, since `/ui` just serves whatever `index.html`
(and any `assets/` alongside it) it finds in that folder.
